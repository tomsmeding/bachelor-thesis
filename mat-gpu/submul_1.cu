...
mp_limb_t
PREFIX(mpn_submul_1) (mp_ptr rp, mp_srcptr up, mp_size_t n, mp_limb_t vl)
{
  ...
  do
    {
      ul = *up++;
      rl = *rp;
      PREFIX(umul_ppmm) (hpl, lpl, ul, shifted_vl);
      lpl >>= GMP_NAIL_BITS;
      PREFIX(SUBC_LIMB) (c1, xl, rl, prev_hpl);
      PREFIX(SUBC_LIMB) (c2, xl, xl, lpl);
      PREFIX(SUBC_LIMB) (c3, xl, xl, cl);
      cl = c1 + c2 + c3;
      *rp++ = xl;
      prev_hpl = hpl;
    }
